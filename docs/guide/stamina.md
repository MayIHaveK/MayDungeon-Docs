...
- 组队进入副本时，每位成员都必须有足够体力。
- 拥有 bypass 权限的玩家不检查也不消耗体力。
- 所有进入条件通过后才会扣除体力；世界创建失败时会自动退还。
- 修改恢复模式或上限后，可用 `/md admin stamina <玩家> reset` 让指定玩家立即按新上限重置。
- 更换存储后端不会自动迁移体力数据，操作前请停服并备份。
